...
class OwnerController extends Controller
{
        ...
        $request->validate ([
            ...
            'email' => [
                ...
                function ($attribute, $value, $fail) use ($dl, $oldEmail, $id) {
                    if ($value !== $oldEmail && $dl->checkExistence('owner', 'email', $value)) {
                        $fail(':attribute must be unique.');
                    }
                    if ($value !== $oldEmail && $dl->checkExistence('customer', 'email', $value)) {
                        $fail(':attribute must be unique.');
                    }
                },
            ],
    ...
    return Redirect::to(route('owner.restaurantinfo', $owner->id))->withInput()->withErrors($errorMessages);
    }
    }
